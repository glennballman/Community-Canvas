  ...
  try {
    await withServiceTransaction(async (client: PoolClient) => {
      const existingCheck = await client.query(
        `SELECT id FROM cc_portals WHERE slug = 'save-paradise-parking' LIMIT 1`
      );
      ...
      console.log('[SEED] Seeding cc_portals for production...');
      ...
      await client.query(`
        INSERT INTO cc_portals (owning_tenant_id, name, slug, portal_type, legal_dba_name, status, primary_audience, tagline)
      ...
      await client.query(`
        ...
        ON CONFLICT (slug) DO UPDATE SET status = 'active'
      `, [tenant1252093Id]);
      console.log('[SEED] Created 1252093 BC LTD cc_portals (Enviropaving, Remote Serve, Enviro Bright)');

      const countResult = await client.query(`SELECT COUNT(*) as count FROM cc_portals`);
      console.log(`[SEED] Portal seeding complete. Total cc_portals: ${countResult.rows[0].count}`);
    });
  } catch (error) {
    console.error('[SEED] Error seeding cc_portals:', error);
  }
}
