...
export async function postPublicAllocationCancelRequest(req: Request, res: Response) {
  try {
    ...
    // Store cancellation request in cart item (request-only, no auto-cancel)
    await db.execute(sql`
      UPDATE cc_reservation_cart_items
      SET cancel_request_reason = ${reason || null},
          cancel_request_message = ${message || null},
    ...
    return res.json(p2Ok({
      ...
      message: "Allocation cancellation request submitted. Staff will review and process."
    }));
