...
router.put('/properties/:id', requireAuth, requireTenant, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await tenantReq.tenantQuery!(
      ...
      WHERE id = $1
      RETURNING *`,
      [id, name, place_type, owner_person_id, owner_organization_id, address_line1, address_line2, city, province, postal_code, country, latitude, longitude, notes]
    );
...
router.get('/contacts', requireAuth, requireTenant, async (req: Request, res: Response) => {
  ...
  try {
    ...
    let whereClause = '';
    if (search) {
      whereClause = `WHERE (c.given_name ILIKE $${paramIndex} OR c.family_name ILIKE $${paramIndex} OR c.email ILIKE $${paramIndex} OR c.telephone ILIKE $${paramIndex})`;
      params.push(`%${search}%`);
      paramIndex++;
    }
...
router.put('/contacts/:id', requireAuth, requireTenant, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await tenantReq.tenantQuery!(
      ...
      WHERE id = $1
      RETURNING *`,
      [id, given_name, family_name, display_name, telephone, email, role_title, organization_id, address_line1, address_line2, city, province, postal_code, country, notes]
    );
...
router.get('/organizations', requireAuth, requireTenant, async (req: Request, res: Response) => {
  ...
  try {
    ...
    let whereClause = '';
    if (search) {
      whereClause = `WHERE (o.name ILIKE $${paramIndex} OR o.email ILIKE $${paramIndex} OR o.telephone ILIKE $${paramIndex})`;
      params.push(`%${search}%`);
      paramIndex++;
    }
...
router.put('/organizations/:id', requireAuth, requireTenant, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await tenantReq.tenantQuery!(
      ...
      WHERE id = $1
      RETURNING *`,
      [id, name, legal_name, telephone, email, website, address_line1, address_line2, city, province, postal_code, country, notes]
    );
