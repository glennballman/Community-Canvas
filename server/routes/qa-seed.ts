  ...
  try {
    ...
    if (crane && transportForDegraded) {
      await withServiceTransaction(async (client) => {
        await client.query(`
          UPDATE unified_assets SET operational_status = 'out_of_service' WHERE id = $1
        `, [crane.id]);
        await client.query(`
      ...
      const parentAfterD = await serviceQuery(`
        SELECT operational_status FROM unified_assets WHERE id = $1
      ...
      const childrenLinks = await serviceQuery(`
        ...
        JOIN unified_assets ua ON ua.id = ac.child_asset_id
      ...
      await withServiceTransaction(async (client) => {
        await client.query(`
          UPDATE unified_assets SET operational_status = 'operational' WHERE id = $1
        `, [crane.id]);
      });
    }
    ...
    await withServiceTransaction(async (client) => {
      await client.query(`
        DELETE FROM asset_children WHERE parent_asset_id IN (
          SELECT id FROM unified_assets WHERE name LIKE 'QA Test%'
        ) OR child_asset_id IN (
          SELECT id FROM unified_assets WHERE name LIKE 'QA Test%'
        )
      `);
      await client.query(`
        DELETE FROM unified_assets WHERE name LIKE 'QA Test%'
      `);
    });
