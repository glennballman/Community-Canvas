# REPLIT: Migration 119 — Wallet Ledger Spine

## Objective
Create the stored value accounting system with append-only ledger entries, hold/capture/release semantics, and available vs posted balance tracking.

## Design Invariants (From ChatGPT)
- **`cc_wallet_entries` is append-only** (no UPDATE/DELETE; reversal is a new entry referencing the original)
- **Available vs Posted**:
  - `posted_balance_cents` = sum of **posted** entries
  - `available_balance_cents` = `posted_balance_cents - active_holds_cents`
  - Holds *reserve* funds; capture converts hold → posted debit entry
- Tenant isolation via `current_tenant_id()` / `is_service_mode()`
- No forbidden terminology (no "booking", "booked", etc.)

## What This Migration Creates

| Component | Count | Details |
|-----------|-------|---------|
| Tables | 4 | `cc_wallet_accounts`, `cc_wallet_entries`, `cc_wallet_holds`, `cc_wallet_balance_snapshots` |
| Enums | 4 | `wallet_account_status`, `wallet_entry_type`, `wallet_entry_status`, `wallet_hold_status` |
| Functions | 6 | Account create, entry post, holds place/capture/release, balance recompute |
| Append-only | 1 | `cc_wallet_entries` with FORCE ROW LEVEL SECURITY |
...
WHERE table_schema = 'public' 
  AND table_name IN ('cc_wallet_accounts', 'cc_wallet_entries', 'cc_wallet_holds', 'cc_wallet_balance_snapshots')
ORDER BY table_name;
```
**Expected:** 4 rows
...
SELECT typname 
FROM pg_type 
WHERE typname IN ('wallet_account_status', 'wallet_entry_type', 'wallet_entry_status', 'wallet_hold_status')
ORDER BY typname;
```
**Expected:** 4 rows

### 3. Functions Created (6)
```sql
SELECT routine_name 
FROM information_schema.routines 
WHERE routine_schema = 'public' 
  AND routine_name IN (
    'cc_create_wallet_account',
    'cc_recompute_wallet_account_balances', 
    'cc_post_wallet_entry',
    'cc_place_wallet_hold',
    'cc_capture_wallet_hold',
    'cc_release_wallet_hold'
  )
...
WHERE schemaname = 'public' 
  AND tablename IN ('cc_wallet_accounts', 'cc_wallet_entries', 'cc_wallet_holds', 'cc_wallet_balance_snapshots')
ORDER BY tablename;
```
**Expected:** All 4 show `rowsecurity = true`

### 5. FORCE RLS on Append-Only Table
```sql
SELECT relname, relforcerowsecurity 
FROM pg_class 
WHERE relname = 'cc_wallet_entries';
```
...
UPDATE cc_wallet_entries SET description = 'hacked' WHERE 1=1;
DELETE FROM cc_wallet_entries WHERE 1=1;
...
DECLARE
  ...
  v_entry_id UUID;
  v_hold_id UUID;
  v_captured_entry_id UUID;
BEGIN
  -- Get first tenant
  SELECT id INTO v_tenant_id FROM cc_tenants LIMIT 1;
  
  -- Set service mode for testing
  PERFORM set_config('app.tenant_id', '__SERVICE__', true);
  
  -- 1. Create wallet account
  v_wallet_id := cc_create_wallet_account(
    v_tenant_id,
    'Test Wallet',
    NULL,
    NULL,
  ...
  -- 2. Post initial credit (deposit $100)
  v_entry_id := cc_post_wallet_entry(
    v_tenant_id,
    v_wallet_id,
    'credit',
  ...
  RAISE NOTICE 'Posted credit entry: %', v_entry_id;
  
  -- 3. Check balance
  RAISE NOTICE 'Balance after deposit:';
  PERFORM posted_balance_cents, available_balance_cents, active_holds_cents
  FROM cc_wallet_accounts WHERE id = v_wallet_id;
  
  -- 4. Place a hold ($25)
  v_hold_id := cc_place_wallet_hold(
    v_tenant_id,
    v_wallet_id,
    2500,  -- $25.00 in cents
    'pending_purchase',
  ...
  RAISE NOTICE 'Placed hold: %', v_hold_id;
  
  -- 5. Check balance (available should be reduced)
  RAISE NOTICE 'Balance after hold:';
  PERFORM posted_balance_cents, available_balance_cents, active_holds_cents
  FROM cc_wallet_accounts WHERE id = v_wallet_id;
  
  -- 6. Capture the hold
  v_captured_entry_id := cc_capture_wallet_hold(
    v_tenant_id,
    v_hold_id,
    'Purchase completed',
    'purchase',
  ...
  RAISE NOTICE 'Captured hold, created entry: %', v_captured_entry_id;
  ...
  -- DELETE FROM cc_wallet_accounts WHERE id = v_wallet_id;
  
  RAISE NOTICE 'Functional test complete!';
END $$;
```
...
After running Migration 119, please report:
...
| Functions created | ? / 6 |
| RLS enabled | ? / 4 |
| FORCE RLS on cc_wallet_entries | ✅ / ❌ |
| Append-only verified | ✅ / ❌ |
| New table count | ? (expected: 512) |

---

## Next Step After Migration 119

**Migration 120: RTR Connector Pack** - adds `cc_rtr_profiles`, `cc_rtr_message_log`, `cc_rtr_webhook_inbox` for Payments Canada Real-Time Rail integration.
