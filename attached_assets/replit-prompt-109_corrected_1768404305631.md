# PROMPT 29: Activity Feed & Event Stream

## Context
- Previous migration: 108_terminology_reservation_cleanup.sql
- This is migration 109
- Builds on notifications (107), jobs (102), invitations (104), value events (103)
- Enables real-time activity visibility for engagement and social proof

## Objective
Create a unified activity feed system that surfaces relevant events to users based on their context (tenant, portal, community). The feed should drive engagement, provide social proof, and support the viral growth loop.

## Design Principles
1. **Context-aware** - Feed content adapts to viewer's role and permissions
2. **Privacy-respecting** - Only show what the viewer is allowed to see
3. **Engagement-driving** - Surface actionable items prominently
4. **Real-time capable** - Support polling and future WebSocket integration
5. **Aggregatable** - Similar events can be grouped ("3 new applications")

---

## Migration: server/migrations/109_activity_feed.sql
...
CREATE INDEX idx_cc_activity_events_tenant ON cc_activity_events(tenant_id, occurred_at DESC);
CREATE INDEX idx_cc_activity_events_portal ON cc_activity_events(portal_id, occurred_at DESC);
CREATE INDEX idx_cc_activity_events_community ON cc_activity_events(community_id, occurred_at DESC);
CREATE INDEX idx_cc_activity_events_verb ON cc_activity_events(verb, occurred_at DESC);
CREATE INDEX idx_cc_activity_events_entity ON cc_activity_events(entity_type, entity_id);
CREATE INDEX idx_cc_activity_events_aggregation ON cc_activity_events(aggregation_key, occurred_at DESC)
  WHERE aggregation_key IS NOT NULL;
CREATE INDEX idx_cc_activity_events_recent ON cc_activity_events(occurred_at DESC)
...
CREATE INDEX idx_cc_activity_feed_state_tenant ON cc_activity_feed_state(tenant_id);
CREATE INDEX idx_cc_activity_feed_state_party ON cc_activity_feed_state(party_id);
CREATE INDEX idx_cc_activity_feed_state_individual ON cc_activity_feed_state(individual_id);
...
CREATE UNIQUE INDEX cc_activity_bookmarks_tenant_unique 
  ON cc_activity_bookmarks(tenant_id, activity_id) WHERE tenant_id IS NOT NULL;
CREATE UNIQUE INDEX cc_activity_bookmarks_party_unique 
  ON cc_activity_bookmarks(party_id, activity_id) WHERE party_id IS NOT NULL;
CREATE UNIQUE INDEX cc_activity_bookmarks_individual_unique 
  ON cc_activity_bookmarks(individual_id, activity_id) WHERE individual_id IS NOT NULL;

CREATE INDEX idx_cc_activity_bookmarks_tenant ON cc_activity_bookmarks(tenant_id, created_at DESC);
CREATE INDEX idx_cc_activity_bookmarks_party ON cc_activity_bookmarks(party_id, created_at DESC);
CREATE INDEX idx_cc_activity_bookmarks_individual ON cc_activity_bookmarks(individual_id, created_at DESC);
...
export const ccActivityEvents = pgTable("cc_activity_events", {
  ...
  entityType: text("entity_type").notNull(), // job, reservation, invitation, portal, etc.
  entityId: uuid("entity_id"),
  subtype: text("subtype"), // Optional refinement
  
  // Actor (XOR identity)
  actorTenantId: uuid("actor_tenant_id").references(() => ccTenants.id, { onDelete: "set null" }),
  actorPartyId: uuid("actor_party_id").references(() => ccParties.id, { onDelete: "set null" }),
  actorIndividualId: uuid("actor_individual_id").references(() => ccIndividuals.id, { onDelete: "set null" }),
  ...
  communityId: uuid("community_id").references(() => ccCommunities.id, { onDelete: "set null" }),
  
  // Content
  title: text("title").notNull(),
  description: text("description"),
  entityName: text("entity_name"), // Denormalized
...
// XOR identity: tenant OR party OR individual
// ============================================================
...
export const ccActivityBookmarks = pgTable("cc_activity_bookmarks", {
  ...
  activityId: uuid("activity_id").notNull().references(() => ccActivityEvents.id, { onDelete: "cascade" }),
  note: text("note"),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({
...
1. [ ] Migration 109 runs without errors
2. [ ] cc_activity_events table created with entity_type/entity_id/subtype columns
3. [ ] cc_activity_feed_state table created with XOR identity (tenant/party/individual)
4. [ ] cc_activity_bookmarks table created with XOR identity
5. [ ] All enums created idempotently (activity_verb, activity_visibility, activity_priority)
6. [ ] Helper functions created with SECURITY DEFINER + SET search_path + guards:
   - cc_record_activity (SERVICE_ONLY)
   - cc_get_activity_feed (recipient_type + scope guards)
   - cc_mark_feed_seen (recipient_type + scope guards)
   - cc_toggle_activity_bookmark (recipient_type + scope guards)
   - cc_get_activity_summary (recipient_type + scope guards)
7. [ ] RLS policies support visibility levels AND party/individual scopes
8. [ ] XOR constraints enforced on feed_state and bookmarks tables
...
WHERE table_schema = 'public' 
  AND table_name LIKE 'cc_activity%';

-- Verify enums created (note: activity_verb not activity_type)
SELECT typname FROM pg_type 
WHERE typname IN ('activity_verb', 'activity_visibility', 'activity_priority');
...
WHERE routine_schema = 'public' 
  AND routine_name IN (
    'cc_record_activity', 'cc_get_activity_feed',
    'cc_mark_feed_seen', 'cc_toggle_activity_bookmark',
...
WHERE schemaname = 'public' 
  AND tablename LIKE 'cc_activity%';

-- Count activity verbs in enum
SELECT COUNT(*) FROM pg_enum 
WHERE enumtypid = 'activity_verb'::regtype;
-- Expected: 21

-- Verify XOR constraint exists on feed_state
SELECT conname FROM pg_constraint 
WHERE conrelid = 'cc_activity_feed_state'::regclass 
  AND conname LIKE '%exactly_one%';

-- Verify XOR constraint exists on bookmarks
SELECT conname FROM pg_constraint 
WHERE conrelid = 'cc_activity_bookmarks'::regclass 
  AND conname LIKE '%exactly_one%';
```
...
### Record Job Posted Activity (Service Mode)
```sql
SET app.tenant_id = '__SERVICE__';

SELECT cc_record_activity(
  p_verb := 'created',
  p_entity_type := 'job',
  p_title := 'New job: Seasonal Groundskeeper',
  p_entity_id := 'job-uuid',
  p_entity_name := 'Seasonal Groundskeeper',
  p_actor_tenant_id := 'employer-tenant-uuid',
  p_community_id := 'bamfield-uuid',
  p_visibility := 'community',
  p_is_actionable := true,
  p_action_url := '/jobs/job-uuid',
  p_action_label := 'View Job'
);
```

### Record Invitation Accepted (Service Mode)
```sql
SET app.tenant_id = '__SERVICE__';
...
SELECT cc_record_activity(
  ...
  p_title := 'John Smith accepted your invitation',
  p_entity_id := 'invitation-uuid',
  p_actor_tenant_id := 'new-tenant-uuid',
  p_tenant_id := 'inviter-tenant-uuid',
  p_visibility := 'tenant'
);
```
...
```sql
SELECT cc_get_activity_feed(
  p_recipient_type := 'tenant',
  p_recipient_id := 'tenant-uuid',
  p_limit := 20,
  p_since := now() - interval '7 days'
);
```

### Get Activity Feed for Individual Worker
```sql
SELECT cc_get_activity_feed(
  p_recipient_type := 'individual',
  p_recipient_id := 'individual-uuid',
  p_entity_type := 'job',  -- Filter to job activities only
  p_limit := 10
);
```
...
```sql
SELECT cc_get_activity_summary(
  p_recipient_type := 'tenant',
  p_recipient_id := 'tenant-uuid',
