# PROMPT 26: Portal Governance

## ChatGPT Fixes Applied (January 14, 2026)

| # | Issue | Fix Applied |
|---|-------|-------------|
| 1 | SECURITY DEFINER search_path injection | Added `SET search_path = public, pg_temp` to all SECURITY DEFINER functions |
| 2 | Identity constraint too loose | Changed to XOR (exactly one of tenant/party/individual) |
| 3 | RLS inconsistency | Created `current_tenant_id()` and `is_service_mode()` helpers, used consistently |
| 4 | Public read on settings | Kept (intentional for branding), added explicit comment |
| 5 | Moderation queue uniqueness | Already have UNIQUE constraint, verified |
| 6 | Function permission model | Added caller verification in `cc_add_portal_member()` and `cc_moderate_content()` |

## Context
- Previous migration: 104_invitations_and_referrals.sql
- This is migration 105
- Builds on existing cc_portals table
- Enables Adrenaline Canada and future portal management

## Objective
Create the governance layer for portals - admin roles, membership, content moderation, and feature configuration. Each portal is a sovereign entity that can operate independently while sharing platform infrastructure.

## Design Principles
1. **Portals are sovereign** - Each portal has its own admins, branding, and governance rules
2. **Federated content** - Jobs/listings can be shared across portals via syndication
3. **Progressive disclosure** - Content goes through approval before public visibility
4. **Silent moderation** - Rejected content disappears without notification (configurable)
5. **Multi-portal membership** - A tenant can participate in multiple portals

---

## Migration: server/migrations/105_portal_governance.sql
...
CREATE INDEX idx_cc_portal_members_portal ON cc_portal_members(portal_id);
CREATE INDEX idx_cc_portal_members_tenant ON cc_portal_members(tenant_id);
CREATE INDEX idx_cc_portal_members_party ON cc_portal_members(party_id);
CREATE INDEX idx_cc_portal_members_role ON cc_portal_members(portal_id, role);
CREATE INDEX idx_cc_portal_members_active ON cc_portal_members(portal_id, is_active) WHERE is_active = true;
...
CREATE INDEX idx_cc_moderation_queue_portal ON cc_moderation_queue(portal_id);
CREATE INDEX idx_cc_moderation_queue_status ON cc_moderation_queue(portal_id, status);
CREATE INDEX idx_cc_moderation_queue_pending ON cc_moderation_queue(portal_id, status, priority DESC) 
  WHERE status = 'pending';
CREATE INDEX idx_cc_moderation_queue_content ON cc_moderation_queue(content_type, content_id);
CREATE INDEX idx_cc_moderation_queue_submitter ON cc_moderation_queue(submitted_by_tenant_id);
...
CREATE INDEX idx_cc_moderation_history_queue ON cc_moderation_history(queue_item_id);
CREATE INDEX idx_cc_moderation_history_actor ON cc_moderation_history(actor_member_id);
CREATE INDEX idx_cc_moderation_history_action ON cc_moderation_history(action);
...
export const ccPortalMembers = pgTable("cc_portal_members", {
  ...
  tenantId: uuid("tenant_id").references(() => ccTenants.id, { onDelete: "cascade" }),
...
export const ccModerationQueue = pgTable("cc_moderation_queue", {
  ...
  submittedByTenantId: uuid("submitted_by_tenant_id").references(() => ccTenants.id),
  submittedByPartyId: uuid("submitted_by_party_id").references(() => ccParties.id),
  submittedByIndividualId: uuid("submitted_by_individual_id").references(() => ccIndividuals.id),
...
1. [ ] Migration 105 runs without errors
2. [ ] cc_portal_members table created with XOR identity constraint (exactly one)
3. [ ] cc_portal_settings table created with all feature flags
4. [ ] cc_moderation_queue table created with unique constraint on (portal_id, content_type, content_id)
5. [ ] cc_moderation_history table created for audit trail
6. [ ] All enums created idempotently (portal_role_type, moderation_status, moderation_action, moderable_content_type)
7. [ ] Helper functions for RLS created: current_tenant_id(), is_service_mode()
8. [ ] Content functions created with SECURITY DEFINER + SET search_path:
   - cc_submit_for_moderation (with permission check)
   - cc_moderate_content (with permission check)
   - cc_has_portal_permission
   - cc_add_portal_member (with permission check, prevents self-admin)
9. [ ] RLS enabled on all 4 tables using current_tenant_id() helper
10. [ ] Portal admins can manage members
11. [ ] Moderators can access moderation queue
12. [ ] Silent rejection is default (configurable)
13. [ ] Default settings seeded for existing portals
14. [ ] Function permission model prevents "add myself as admin" attacks
15. [ ] Drizzle schema updated and synced
...
WHERE table_schema = 'public' 
  AND table_name IN ('cc_portal_members', 'cc_portal_settings', 'cc_moderation_queue', 'cc_moderation_history');
...
WHERE routine_schema = 'public' 
  AND routine_name IN ('current_tenant_id', 'is_service_mode');

-- Verify content functions with SECURITY DEFINER
SELECT routine_name, security_type
FROM information_schema.routines 
WHERE routine_schema = 'public' 
  AND routine_name IN (
    'cc_submit_for_moderation', 'cc_moderate_content',
    'cc_has_portal_permission', 'cc_add_portal_member'
  );
-- security_type should be 'DEFINER' for all

-- Verify XOR identity constraint on portal_members
SELECT conname, pg_get_constraintdef(oid) 
FROM pg_constraint 
WHERE conrelid = 'cc_portal_members'::regclass 
  AND conname LIKE '%identity%';

-- Verify default settings seeded for existing portals
SELECT p.name, ps.id IS NOT NULL as has_settings
FROM cc_portals p
LEFT JOIN cc_portal_settings ps ON ps.portal_id = p.id;

-- Verify RLS enabled
SELECT tablename, rowsecurity 
FROM pg_tables 
WHERE schemaname = 'public' 
  AND tablename IN ('cc_portal_members', 'cc_portal_settings', 'cc_moderation_queue', 'cc_moderation_history');

-- Verify moderation queue unique constraint
SELECT conname 
FROM pg_constraint 
WHERE conrelid = 'cc_moderation_queue'::regclass 
  AND contype = 'u';

-- Test permission check (use real portal_id and tenant_id)
-- SELECT cc_has_portal_permission('portal-uuid', 'tenant-uuid', 'moderate');
```

---

## Integration with Jobs Cold-Start

When a job is posted to a portal:

```
1. Job created in cc_jobs
2. Job posting created in cc_job_postings (portal_id set)
3. cc_submit_for_moderation() called with content_type='job_posting'
4. If auto_approve_jobs=true → auto_approved, visible immediately
5. If auto_approve_jobs=false → pending, awaits moderator
6. Moderator calls cc_moderate_content() with 'approve' or 'reject'
7. If rejected with silent_rejection=true → disappears, no notification
```

---

## Portal Governance Flow
