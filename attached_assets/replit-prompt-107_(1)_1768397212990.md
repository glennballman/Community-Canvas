# PROMPT 28: Notifications & Alerts

## ChatGPT Fixes Applied (January 14, 2026)

| # | Issue | Fix Applied |
|---|-------|-------------|
| 1 | Terminology violation: `booking` in enum | Changed to `reservation` everywhere |
| 2 | NULL prefs crash in cc_create_notification | Added `COALESCE(v_prefs.digest_frequency, 'immediate')` |
| 3 | cc_create_notification missing guard | Added `SERVICE_ONLY` guard |
| 4 | Party/individual recipients not enforceable | Added `cc_tenant_individuals` table + updated RLS + updated all functions to support recipient_type |

**Key architectural decision:** Implemented Option B fully - party/individual support is complete and enforceable, no refactor needed later.

## Context
- Previous migration: 106_onboarding_wizard.sql
- This is migration 107
- Builds on invitations (104), moderation (105), onboarding (106)
- Enables multi-channel notification delivery

## Objective
Create a unified notification system that delivers alerts across multiple channels (in-app, email, SMS, push) with user preferences, delivery tracking, and batching support. Notifications should be actionable and context-aware.

## Design Principles
1. **Channel-agnostic** - Same notification can go to multiple channels
2. **User-controlled** - Preferences determine what/when/how
3. **Actionable** - Notifications link to relevant actions
4. **Batchable** - Digest mode for high-volume users
5. **Auditable** - Full delivery tracking
6. **Silent failures** - Don't break workflows if notification fails

---

## Migration: server/migrations/107_notifications_and_alerts.sql
...
CREATE INDEX idx_cc_tenant_individuals_tenant ON cc_tenant_individuals(tenant_id);
CREATE INDEX idx_cc_tenant_individuals_individual ON cc_tenant_individuals(individual_id);
...
CREATE INDEX idx_cc_notification_templates_code ON cc_notification_templates(code);
CREATE INDEX idx_cc_notification_templates_category ON cc_notification_templates(category);
...
CREATE INDEX idx_cc_notification_preferences_tenant ON cc_notification_preferences(tenant_id);
CREATE INDEX idx_cc_notification_preferences_party ON cc_notification_preferences(party_id);
CREATE INDEX idx_cc_notification_preferences_individual ON cc_notification_preferences(individual_id);
...
CREATE INDEX idx_cc_notifications_recipient_tenant ON cc_notifications(recipient_tenant_id);
CREATE INDEX idx_cc_notifications_recipient_party ON cc_notifications(recipient_party_id);
CREATE INDEX idx_cc_notifications_status ON cc_notifications(status);
CREATE INDEX idx_cc_notifications_category ON cc_notifications(category);
CREATE INDEX idx_cc_notifications_pending ON cc_notifications(status, scheduled_for) 
  WHERE status IN ('pending', 'queued');
CREATE INDEX idx_cc_notifications_unread ON cc_notifications(recipient_tenant_id, status, created_at) 
  WHERE status NOT IN ('read', 'cancelled');
CREATE INDEX idx_cc_notifications_context ON cc_notifications(context_type, context_id);
...
CREATE INDEX idx_cc_notification_deliveries_notification ON cc_notification_deliveries(notification_id);
CREATE INDEX idx_cc_notification_deliveries_status ON cc_notification_deliveries(status);
CREATE INDEX idx_cc_notification_deliveries_provider ON cc_notification_deliveries(provider_message_id);
...
CREATE INDEX idx_cc_notification_digests_recipient ON cc_notification_digests(recipient_tenant_id);
CREATE INDEX idx_cc_notification_digests_pending ON cc_notification_digests(status, period_end) 
...
BEGIN
  ...
  SELECT 
    v_notification_id,
    unnest(v_channels),
    CASE unnest(v_channels)
      WHEN 'email' THEN v_prefs.email_address
      WHEN 'sms' THEN v_prefs.phone_number
      WHEN 'push' THEN v_prefs.push_token
      ELSE NULL
    END;
...
export const ccTenantIndividuals = pgTable("cc_tenant_individuals", {
  ...
  individualId: uuid("individual_id").notNull().references(() => ccIndividuals.id, { onDelete: "cascade" }),
  role: text("role"),
...
export const ccNotificationPreferences = pgTable("cc_notification_preferences", {
  id: uuid("id").primaryKey().defaultRandom(),
  tenantId: uuid("tenant_id").references(() => ccTenants.id, { onDelete: "cascade" }).unique(),
  partyId: uuid("party_id").references(() => ccParties.id, { onDelete: "cascade" }).unique(),
  individualId: uuid("individual_id").references(() => ccIndividuals.id, { onDelete: "cascade" }).unique(),
...
export const ccNotifications = pgTable("cc_notifications", {
  ...
  recipientTenantId: uuid("recipient_tenant_id").references(() => ccTenants.id, { onDelete: "cascade" }),
  recipientPartyId: uuid("recipient_party_id").references(() => ccParties.id, { onDelete: "cascade" }),
  recipientIndividualId: uuid("recipient_individual_id").references(() => ccIndividuals.id, { onDelete: "cascade" }),
  ...
  actionLabel: text("action_label"),
  senderTenantId: uuid("sender_tenant_id").references(() => ccTenants.id),
...
export const ccNotificationDeliveries = pgTable("cc_notification_deliveries", {
  ...
  uniqueDelivery: uniqueIndex("cc_notification_deliveries_unique").on(table.notificationId, table.channel),
}));
...
export const ccNotificationDigests = pgTable("cc_notification_digests", {
  ...
  uniqueDigest: uniqueIndex("cc_notification_digests_unique").on(table.recipientTenantId, table.frequency, table.periodStart),
}));
...
1. [ ] Migration 107 runs without errors
2. [ ] **cc_tenant_individuals table created** (foundational scope primitive for party/individual)
3. [ ] cc_notification_templates table created with 10 seeded templates
4. [ ] cc_notification_preferences table created with XOR identity constraint
5. [ ] cc_notifications table created with XOR recipient constraint
6. [ ] cc_notification_deliveries table created for per-channel tracking
7. [ ] cc_notification_digests table created for batched delivery
8. [ ] All enums created idempotently (5 enums, with `reservation` not `booking`)
9. [ ] Helper functions created with SECURITY DEFINER + SET search_path:
   - cc_create_notification (SERVICE_ONLY guard)
   - cc_mark_notification_read (recipient_type + recipient_id + scope guard)
   - cc_mark_all_notifications_read (recipient_type + recipient_id + scope guard)
   - cc_get_unread_notification_count (recipient_type + recipient_id + scope guard)
   - cc_get_notifications (recipient_type + recipient_id + scope guard)
   - cc_update_notification_preferences (recipient_type + recipient_id + scope guard)
10. [ ] RLS policies support tenant/party/individual recipients via tenant scope
11. [ ] cc_create_notification uses COALESCE for digest_frequency (NULL prefs fix)
12. [ ] 10 notification templates seeded
...
```sql
-- Verify tables exist (including cc_tenant_individuals)
SELECT table_name 
FROM information_schema.tables 
WHERE table_schema = 'public' 
  AND (table_name LIKE 'cc_notification%' OR table_name = 'cc_tenant_individuals');

-- Verify enums created (reservation not booking)
...
-- Verify reservation is in notification_category (not booking)
SELECT enumlabel FROM pg_enum 
WHERE enumtypid = 'notification_category'::regtype
ORDER BY enumsortorder;

-- Verify cc_tenant_individuals has RLS
SELECT tablename, rowsecurity 
FROM pg_tables 
WHERE tablename = 'cc_tenant_individuals';
...
WHERE routine_schema = 'public' 
  AND routine_name IN (
    'cc_create_notification', 'cc_mark_notification_read',
    'cc_mark_all_notifications_read', 'cc_get_unread_notification_count',
    'cc_get_notifications', 'cc_update_notification_preferences'
  );
...
FROM pg_constraint 
WHERE conrelid IN ('cc_notification_preferences'::regclass, 'cc_notifications'::regclass)
  AND conname LIKE '%identity%' OR conname LIKE '%recipient%';

-- Verify RLS enabled on all notification tables
SELECT tablename, rowsecurity 
FROM pg_tables 
WHERE schemaname = 'public' 
  AND tablename LIKE 'cc_notification%';

-- Test get notifications with recipient_type (use real ids)
-- SELECT cc_get_notifications('tenant', 'tenant-uuid', 20, 0, NULL, false);
```
...
### Send Invitation Notification (Service Mode)
```sql
-- Run in service mode
SET app.tenant_id = '__SERVICE__';

SELECT cc_create_notification(
  p_recipient_tenant_id := 'tenant-uuid',
  p_template_code := 'invitation_received',
  p_context_type := 'invitation',
  p_context_id := 'invitation-uuid',
  p_context_data := '{
    "context_name": "Housekeeper at Bamfield",
    "invitee_role": "worker",
    "claim_token": "abc123..."
  }'::jsonb
);
...
### Send Job Application Notification (Service Mode)
```sql
SET app.tenant_id = '__SERVICE__';

SELECT cc_create_notification(
  p_recipient_tenant_id := 'employer-tenant-uuid',
  p_template_code := 'job_application_received',
  p_context_type := 'job',
  p_context_id := 'job-uuid',
  p_context_data := '{
    "job_title": "Seasonal Groundskeeper",
...
### Get Notifications for UI (Tenant Context)
```sql
-- For tenant recipients
SELECT cc_get_notifications(
...
-- For individual recipients (worker in tenant's org)
SELECT cc_get_notifications(
...
```sql
SELECT cc_mark_notification_read(
  p_recipient_type := 'tenant',
  p_recipient_id := 'tenant-uuid',
...
### Update Preferences
```sql
SELECT cc_update_notification_preferences(
  p_recipient_type := 'tenant',
  p_recipient_id := 'tenant-uuid',
  p_preferences := '{
    "email_enabled": true,
