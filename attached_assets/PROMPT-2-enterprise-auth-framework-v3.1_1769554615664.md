# REPLIT PROMPT — PROMPT-2
## Enterprise Authorization Framework v3.1 (FINAL)

ROLE:
Senior Platform Architect + Security & Authorization Authority

AUTHORITY:
This prompt is governed by AUTH_CONSTITUTION.md.
No scope reduction or reinterpretation is permitted.

---

## OBJECTIVE

Implement a **capability-first, enterprise-grade authorization system** that supports:
...
### A. Principals (Unified Actors)

Create `cc_principals` as the single actor abstraction.

Each principal MUST represent exactly one of:
- Human user
- Service account
- Machine
- Delegate (impersonation / proxy)

Authorization resolution MUST be identical for all principals.

---

### B. Capability-First Authorization

Create atomic capabilities as the ONLY permission source.

Roles:
- Are convenience bundles
- MUST resolve to capabilities
- MUST NOT be checked directly

---

### C. Scope Hierarchy (MANDATORY)

Authorization MUST evaluate the full hierarchy:
...
Implementations that shortcut at tenant scope are INVALID.

---

### D. Ownership & Own/All Pattern
...
Rules:
- Ownership MUST be enforced via RLS
- `own` capabilities MUST enforce:
  `created_by_principal_id = current_principal_id`
- JSON conditions MAY refine behavior
- JSON conditions MAY NOT replace RLS ownership enforcement

---

### E. Machine Safety Enforcement
...
If unmet:
- Authorization MUST hard-fail
- No warnings
- No UI-only gating

Emergency stop capabilities MUST be universally available.

---

### F. External System Mappings
...
Mappings MUST translate external roles → CC roles → CC capabilities.

---

### G. Jobber Mapping (Validated)
...
These mappings MUST be enforced via capabilities, not flags.

---

### H. Resource-Level Permissions

Implement:
- Attribute-based ownership (created_by)
- Explicit grants (`cc_resource_grants`)

Both MUST integrate with RLS.

---

### I. Claim Framework (Merged)

Tenants may exist in:
- Unclaimed
- Claim-Pending
- Claimed
...
Strong methods MAY auto-approve.
Weak methods MUST escalate.

---

### J. Migration Safety

Migration from legacy systems MUST:
- Be additive
- Avoid dual-write
- Fail loudly if legacy paths are used
...
**File:** server/db/migrations/0161_enterprise_auth_framework.sql
...
-- Auto-create scope for organizations
...
BEGIN
    ...
    VALUES (
        ...
        'platform/org:' || COALESCE(NEW.slug, NEW.id::text)
    )
    ON CONFLICT DO NOTHING;
    RETURN NEW;
END;
...
BEGIN
    IF NEW.organization_id IS NOT NULL THEN
        ...
        FROM cc_scopes 
        WHERE scope_type = 'organization' AND organization_id = NEW.organization_id;
        v_parent_scope_id := COALESCE(v_org_scope_id, '00000000-0000-0000-0000-000000000001');
    ELSE
        v_parent_scope_id := '00000000-0000-0000-0000-000000000001';
    END IF;
    ...
    VALUES (
        ...
        CASE 
            WHEN NEW.organization_id IS NOT NULL THEN 
                (SELECT scope_path FROM cc_scopes WHERE id = v_parent_scope_id) || '/tenant:' || COALESCE(NEW.slug, NEW.id::text)
            ELSE 
                'platform/tenant:' || COALESCE(NEW.slug, NEW.id::text)
        END
    )
    ON CONFLICT DO NOTHING;
    RETURN NEW;
...
-- 5f. Backfill scopes for existing organizations
INSERT INTO cc_scopes (scope_type, organization_id, parent_scope_id, scope_path)
SELECT 
    'organization',
    o.id,
    '00000000-0000-0000-0000-000000000001',
    'platform/org:' || COALESCE(o.slug, o.id::text)
...
-- 5g. Backfill scopes for existing tenants
INSERT INTO cc_scopes (scope_type, tenant_id, parent_scope_id, scope_path)
SELECT 
    'tenant',
    t.id,
    COALESCE(
        (SELECT s.id FROM cc_scopes s WHERE s.scope_type = 'organization' AND s.organization_id = t.organization_id),
        '00000000-0000-0000-0000-000000000001'
    ),
    CASE 
        WHEN t.organization_id IS NOT NULL THEN 
            (SELECT s.scope_path FROM cc_scopes s WHERE s.scope_type = 'organization' AND s.organization_id = t.organization_id) 
            || '/tenant:' || COALESCE(t.slug, t.id::text)
        ELSE 
            'platform/tenant:' || COALESCE(t.slug, t.id::text)
    END
FROM cc_tenants t
WHERE NOT EXISTS (
    SELECT 1 FROM cc_scopes s 
...
### Migration 162: Seed Capabilities

**File:** server/db/migrations/0162_seed_auth_capabilities.sql
...
**File:** server/db/migrations/0163_seed_auth_roles.sql
...
-- Service Provider Manager (Jobber: dispatcher)
INSERT INTO cc_roles (id, code, name, description, is_system_role, external_system, external_role_code)
VALUES ('10000000-0000-0000-0000-000000000005', 'service_provider_manager', 'Service Provider Manager', 'Manage service runs', true, 'jobber', 'dispatcher')
ON CONFLICT DO NOTHING;

-- Field Worker (Jobber: worker)
INSERT INTO cc_roles (id, code, name, description, is_system_role, external_system, external_role_code)
VALUES ('10000000-0000-0000-0000-000000000006', 'field_worker', 'Field Worker', 'Complete own work', true, 'jobber', 'worker')
ON CONFLICT DO NOTHING;
...
WHERE domain IN ('service_runs', 'projects', 'bids', 'people') OR code IN ('team.own.read', 'team.own.update', 'analytics.view')
ON CONFLICT DO NOTHING;

-- Field Worker (own resources only)
INSERT INTO cc_role_capabilities (role_id, capability_id)
SELECT '10000000-0000-0000-0000-000000000006', id FROM cc_capabilities WHERE code LIKE '%.own.%'
ON CONFLICT DO NOTHING;
...
- Capability catalog (60+ capabilities)
- Role bundles (8 system roles)
- External mappings (Jobber, Cloudbeds)
- RLS enforcement on all auth tables
- Verification that all AUTH_CONSTITUTION invariants hold

---

## SUCCESS CRITERIA

✔ Capability-first enforcement everywhere  
✔ No role or flag checks  
✔ Full scope hierarchy enforced (MANDATORY - no shortcuts)  
✔ Machines treated as first-class principals  
✔ Machine safety is HARD-FAIL (no logging/warnings)  
✔ No parallel systems remain  
✔ Ownership enforced via RLS (not UI filtering)

Any deviation is a failed implementation.
