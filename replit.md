...
- **N3 Service Run Monitor + Replan Engine (Patent CC-01)**: Real-time service run monitoring with attention bundles for proactive intervention. Inventor: Glenn Ballman. Tables: cc_service_runs, cc_service_run_runs, cc_service_run_events, cc_service_run_attention_bundles. Evaluator classifies run health based on deviation thresholds. API at `/api/n3/*`. UI at `/app/service-runs/attention` and `/app/service-runs/monitor`. Dev seed at `/api/dev/seed/n3`.
