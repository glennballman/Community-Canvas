...
- **System Explorer**: Permanent debug/discovery surface at `/app/system-explorer` with 5 tabs: Overview (entity counts, integration status), Integrations (Firecrawl, Apify, Mapbox, Jobber, CompanyCam), Data Sources (5 pipelines: DriveBC, BC Ferries, BC Hydro, Weather, Earthquakes), Data Browser (read-only exploration of 17 tables), Routes Audit (nav item verification). API prefix: `/api/admin/system-explorer`. Nav regression lock in `scripts/qa-smoke-test.ts` prevents accidental removal of required nav items.
