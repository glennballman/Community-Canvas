...
- **Application-Layer Auth Enforcement (PROMPT-3)**: `server/auth/` module with fail-closed authorization (`authorize()`, `can()`, `requireCapability()`), single identity authority (`resolvePrincipalFromSession()`), idempotent scope resolution, and comprehensive audit logging to `cc_auth_audit_log`. DB functions: `cc_has_capability()`, `cc_auth_audit_log_insert()`, `get_or_create_tenant_scope()`. `/api/me/context` returns `principal_id` and `effective_principal_id` for impersonation tracking.
- **PROMPT-10 Forbidden Authority Purge**: All production authorization decisions use `cc_grants` exclusively - NO `isPlatformAdmin` flag checks. Capability-based: `can(req, 'platform.configure')`. Grant-based: `checkPlatformAdminGrant(userId)`. Guardrail tests enforce compliance with 9 tests in `forbidden-authority-sources.test.ts`.
