...
- **Table Naming Convention**: All 365 application tables use `cc_*` prefix (e.g., `cc_tenants`, `cc_users`, `cc_reservations`). System tables (`session`, `spatial_ref_sys`) are excluded. All constraints and indexes also use `cc_*` prefix. SQL column aliasing maintains backward compatibility with API contracts.
...
- **Unified Assets Registry**: Central registry unifying all rentable assets (properties, spots, trailers, vehicles, equipment) into a single searchable system with unified reservations and traveler bonds.
...
- **Canadian Tax & Payroll System**: Complete Canadian payroll infrastructure including: `services` (schema.org Service with USALI revenue department links), `tax_jurisdictions` (21 jurisdictions: 14 CA federal+provinces+territories, 7 US states), `tax_rates` (GST/HST/PST rates), `organization_tax_registrations` (BN, GST/HST, provincial tax numbers), `payroll_income_tax_brackets` (66 brackets for 2025 across all provinces), `payroll_contribution_rates` (CPP/CPP2/EI/QPP/QPIP with maximums), `payroll_employer_taxes` (BC EHT, ON EHT, MB HE, QC HSF/WSDRF, NL HAPSET), `employees`, `time_entries`, `pay_runs`, `pay_stubs`, `statutory_holidays` (18 holidays for 2025). Revenue departments: 18 USALI-inspired codes (ROOMS, CHARTER, FIRE, FISHING, etc.).
- **Infrastructure & Organization Schema**: Comprehensive schema.org classification for ~10,000+ entities. Reference tables: `ref_infrastructure_types` (64 types: airports, fire halls, hospitals, schools, utilities, etc. with schema.org types and NAICS codes), `ref_organization_types` (79 types: government, non-profit, hospitality, retail, professional services, trades, etc.). Organizations table extended with: `infrastructure_type`, `organization_type`, `is_infrastructure`, `is_government`, `is_emergency_service`, `is_chamber_member`, `chamber_id`, lat/lng coordinates. Views: `v_infrastructure` (unified infrastructure entities), `v_chamber_members` (chamber membership with organization types).
- **SEO & Sitemap System**: Multi-tenant sitemap infrastructure with portal-aware filtering. Routes: `/sitemap.xml` (main index or portal-specific), `/sitemap-articles.xml`, `/sitemap-organizations.xml`, `/sitemap-infrastructure.xml`. Features: portal domain resolution via `portal_domains` table or subdomain pattern, `Vary: Host, X-Forwarded-Host` headers for CDN cache isolation, defensive URL construction with fallback to slug-based URLs. JSON-LD structured data: `server/lib/schema-org.ts` with functions for Article, Infrastructure, Organization, Breadcrumb, and WebSite types. Articles table enhanced with SEO columns: `content_type`, `meta_title`, `meta_description`, `canonical_url`, `featured_image_url`, `author_id`, `reading_time_minutes`, `published_at`, `summary`.
- **Media Storage System**: Cloudflare R2-based storage for images, documents, and crawled media. Tables: `media` (core storage with schema.org ImageObject metadata), `entity_media` (polymorphic links to any entity), `crawl_media_queue` (queue for downloaded crawled images). Features: automatic image optimization with Sharp (thumbnails, medium variants), presigned URLs for direct browser upload, tenant-isolated storage with RLS. API prefix: `/api/media`. Requires environment variables: `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET_NAME`, `R2_PUBLIC_URL`.
- **Authority & Permits System**: Multi-authority permit management for Parks Canada, First Nations, and Harbour Authorities. Tables: `cc_authorities` (regulatory bodies with cultural protocols), `cc_permit_types` (permit categories with fee structures), `cc_visitor_permits` (individual permits with lifecycle management). Permit number format: `PRM-{AUTHORITY_CODE}-YYMMDD-{4CHAR}`. Status flow: draft → submitted → approved → issued → active → expired. Fee calculation: base + per-person + per-day + per-night components. Features: QR code verification, portal isolation, status transition validation. API prefix: `/api/permits`. Note: `cc_visitor_permits` is separate from `cc_permits` (work order compliance permits).
